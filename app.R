...
nombres <- SISINTAR:::tabla_nombres

nombres_perfil <- nombres[startsWith(nombres$nombre, "s_"), ]
variables_perfil <- setNames(nombres_perfil$nombre, nombres_perfil$titulo)

nombres_horizonte <- nombres[!startsWith(nombres$nombre, "s_"), ]
variables_horizonte <- setNames(nombres_horizonte$nombre, nombres_horizonte$titulo)
variables_horizonte <- variables_horizonte[!(variables_horizonte %in% c("perfil_id", "profundidad_inferior", "profundidad_superior"))]

estandar <- c(0, 5, 15, 30, 60, 100, 200)

nombres <- paste0(estandar[-length(estandar)], "-", estandar[-1], " cm")

horizontes_estandar <- lapply(seq_along(estandar)[-1], function(i) {
  c(profundidad_inferior = estandar[i-1],
    profundidad_superior = estandar[i])
})
names(horizontes_estandar) <- paste0(estandar[-length(estandar)], "-", estandar[-1], " cm")


data.table::transpose(horizontes_estandar)
    ...
    fluidRow(
      box(title = h2("Procesamiento"), width = 12,
          ...
          column(width = 6,
                 ...
                 selectInput("horizontes_interpolar", "Horizontes",
                             choices = names(horizontes_estandar),
                             multiple = TRUE,
                             selected = names(horizontes_estandar)[1])
          )
      )
    )
  )
  ...
  perfiles_estandaraizados <- reactive({
    ...
    if (input$interpolacion != "Ninguna") {
      ...
      if (input$interpolacion == "Splines") {
        malos <- data.table::as.data.table(datos)[, .SD[is.na(profundidad_inferior + profundidad_superior)], by = perfil_id]
        ...
        if (length(malos) > 0) {
          ...
          datos <- datos[!(perfil_id %in% malos), ]

        }
      }

      horizontes_interpolar <- horizontes_estandar[input$horizontes_interpolar]

      variables <- input$variables_horizonte
      if (is.null(variables)) {
        variables <- variables_horizonte
      }


      datos <- lapply(horizontes_interpolar, function(h) {
        SISINTAR::interpolar_perfiles(datos,
                                      variables = variables,
                                      horizontes = h,
                                      metodo = metodo_interpolar)
      })

      datos <- data.table::rbindlist(datos)
    }
